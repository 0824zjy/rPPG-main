    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = EfficientPhys(frame_depth=self.frame_depth,
                                       img_size=config.TRAIN.DATA.PREPROCESS.H).to(self.device)
            ...
            self.model = EfficientPhys(frame_depth=self.frame_depth,
                                       img_size=config.TEST.DATA.PREPROCESS.H).to(self.device)
            self.model = torch.nn.DataParallel(self.model, device_ids=list(range(config.NUM_OF_GPU_TRAIN)))
        else:
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # 裁剪数据使得帧数为 base_len 的整数倍
                data = data[:(N * D) // self.base_len * self.base_len]
                # 为了模型中 torch.diff 的计算，添加最后一帧
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(data_loader['test']):
                ...
                for idx in range(batch_size):
                    subj_index = test_batch[2][idx]  # 如 "01-01"
